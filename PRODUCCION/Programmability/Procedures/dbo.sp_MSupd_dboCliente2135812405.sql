...
GO
CREATE procedure [dbo].[sp_MSupd_dboCliente2135812405]
			@c1 int = NULL,
			@c2 varchar(15) = NULL,
			@c3 varchar(50) = NULL,
			@c4 varchar(50) = NULL,
...
if @Offset < 0 and @Length < 0 and @partcol < 0
	update [dbo].[Cliente] set
			[idCliente] = case substring(@bitmap,1,1) & 1 when 1 then @c1 else [idCliente] end,
			[cedulaCliente] = case substring(@bitmap,1,1) & 2 when 2 then @c2 else [cedulaCliente] end,
			[nombreCliente] = case substring(@bitmap,1,1) & 4 when 4 then @c3 else [nombreCliente] end,
			[apellidoCliente] = case substring(@bitmap,1,1) & 8 when 8 then @c4 else [apellidoCliente] end,
			[telefonoCliente] = case substring(@bitmap,1,1) & 16 when 16 then @c5 else [telefonoCliente] end,
			[direccionCliente] = case substring(@bitmap,1,1) & 32 when 32 then @c6 else [direccionCliente] end,
			[fechaNacimientoCliente] = case substring(@bitmap,1,1) & 64 when 64 then @c7 else [fechaNacimientoCliente] end,
			[idTienda] = case substring(@bitmap,1,1) & 128 when 128 then @c8 else [idTienda] end,
...
update [dbo].[Cliente] set [' + replace(c.name,'''','''''') + N'].write(@c' + convert(nvarchar(10), @partcol) + N', @Offset, @Length), $sys_p2p_cd_id = @MSp2pPostVersion 
	where [idCliente] = @pkc1
  and ($sys_p2p_cd_id = @MSp2pPreVersion or $sys_p2p_cd_id is null)
' when t.name in (N'text', N'ntext') then N'
declare @ptrvald varbinary(16) 
select @ptrvald = textptr([' + replace(c.name,'''','''''') + N']) from [dbo].[Cliente] 
	where [idCliente] = @pkc1
  and ($sys_p2p_cd_id = @MSp2pPreVersion or $sys_p2p_cd_id is null)
if @ptrvald is NULL begin 
	update [dbo].[Cliente] set [' + replace(c.name,'''','''''') + N'] = NULL 
	where [idCliente] = @pkc1
  and ($sys_p2p_cd_id = @MSp2pPreVersion or $sys_p2p_cd_id is null)
...
if @ptrvald is not NULL begin 
	...
	updatetext [dbo].[Cliente].[' + replace(c.name,'''','''''') + N'] @ptrvald @iOffset @iLength with log @c' + convert(nvarchar(10), @partcol) + N'
	update [dbo].[Cliente] set $sys_p2p_cd_id = @MSp2pPostVersion 
	where [idCliente] = @pkc1
end' else NULL end
			...
			where c.object_id = OBJECT_ID(N'[dbo].[Cliente]') and c.column_id = @partcol
	if @update_part_3 is null
		return
	else
		exec sp_executesql
			@update_part_3,
			N' @c1 int, @c2 varchar(15), @c3 varchar(50), @c4 varchar(50), @c5 varchar(15), @c6 varchar(250), @c7 date, @c8 int, @pkc1 int, @MSp2pPreVersion varbinary(32), @MSp2pPostVersion varbinary(32), @Offset bigint, @Length bigint, @partcol int ',
			@c1,
			@c2,
			@c3,
			@c4,
